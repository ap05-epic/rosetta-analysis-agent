...
# Title used by the catch-all playbook entry; it names a symptom, not a cause,
# so it never earns the confidence boost a recognised failure mode gets.
_GENERIC_TITLE = "Application errors (HTTP 5xx)"
...
_PLAYBOOK = [
    # NB: not bare "oom" — it matches "room", "zoom", "bloom".
    (("out of memory", "oom kill", "killed process", "vmem", "heap space", "memory limit"),
     "Memory exhaustion", [
        "Restart the affected service to recover, then review its memory limits.",
        "Cap the query or job that consumed the memory — the cited lines name it.",
        "Capture a heap profile to find the cause before simply raising limits.",
    ]),
    ...
    (("syntax error at or near", "errors.syntaxerror"),
     "Invalid SQL generated by the pipeline", [
        "Fix the malformed statement at the cited lines — the logged SQL shows the exact position.",
        "Validate rendered SQL against the database in staging, or with a dry run, before execution.",
        "Re-run the failed task once the statement is corrected so downstream work can be scheduled.",
    ]),
    (("permission denied", "insufficientprivilege", "forbidden", "unauthorized", "403"),
     "Access denied — credentials or grants", [
        "Confirm the service account still holds the grants named in the cited lines.",
        "Check whether a credential rotation completed on every consumer, not just some.",
        "Re-run the failed job once access is restored; no data change is needed.",
    ]),
    (("no module named", "modulenotfounderror", "importerror"),
     "Missing dependency in the deployed image", [
        "Rebuild and redeploy the image with the missing package pinned in the lock file.",
        "Roll back to the previous image so the job can run while the rebuild happens.",
        "Add an import smoke test to the build so a missing dependency fails at build time.",
    ]),
    (("checksum mismatch", "validation failed", "verification failed", "manifest"),
     "Data validation failure", [
        "Compare the failing artefacts against their manifest before releasing anything downstream.",
        "Re-fetch or regenerate the affected files, then re-run the verification step.",
        "Hold downstream delivery until the mismatch is explained — do not override the check.",
    ]),
    ...
    # NB: not bare "version" — it matches unrelated lines like "secret version 42"
    (("deployment", "rollout", "deployed", "new release", "deploy started"),
     "Faulty deployment", [
...
class MockProvider(LLMProvider):
    ...
    def _final_answer(self, messages: list) -> dict:
        ...
        # A group the playbook recognises names a cause. An unrecognised group is
        # usually framework boilerplate ("Task failed with exception") that repeats
        # more often than the real error — so rank known causes ahead of raw count.
        def _known(g):
            sample = g.get("sample_message") or g["message_signature"]
            title = _playbook_lookup(f"{g['message_signature']} {sample}")[0]
            return title is not None and title != _GENERIC_TITLE

        def _rank(g):
            return (not _known(g), g["level"] not in ("ERROR", "CRITICAL"), -g["count"])

        groups = sorted(groups, key=_rank)
        ...
        for g in groups:
            ...
            inc = {
                ...
                "confidence": (0.95 if _known(g) and g["count"] >= 3
                               else 0.9 if _known(g)
                               else 0.75 if g["count"] >= 3 else 0.7),
                "_count": g["count"],
                "_signature": sample,
